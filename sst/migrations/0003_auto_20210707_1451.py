# Generated by Django 3.2.5 on 2021-07-07 09:21

import datetime
from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('sst', '0002_auto_20210707_1447'),
    ]

    operations = [
        migrations.CreateModel(
            name='TestDetails',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user_id', models.CharField(default='sst21', max_length=50, verbose_name='ID')),
                ('test_name', models.CharField(max_length=50, verbose_name='Name of the Test')),
                ('marks', models.IntegerField()),
                ('created_on', models.DateTimeField(default=datetime.datetime(2021, 7, 7, 14, 51, 22, 570993))),
                ('published_status', models.BooleanField(default=True)),
                ('name', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='Username')),
            ],
            options={
                'verbose_name_plural': 'Test Details',
            },
        ),
        migrations.DeleteModel(
            name='Student',
        ),
    ]
